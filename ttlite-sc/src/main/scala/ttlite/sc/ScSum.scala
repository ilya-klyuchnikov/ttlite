  ...
  override def proofFold(
    ...
    node.outs match {
      case TEdge(nodeL, ElimLabel(sel, InL(et, Free(lN)), _, _)) ::
        ...
        val lCase =
          VLam(aVal, l => proofFold(nodeL, env1 + (lN -> l), l :: bound1, recM1, env2 + (lN -> l), l :: bound2, recM2))

        val rCase =
          VLam(bVal, r => proofFold(nodeR, env1 + (rN -> r), r :: bound1, recM1, env2 + (rN -> r), r :: bound2, recM2))
