...
object Path {
  ...
  def norm(i: Int, n: Int): List[PathElem] =
    (i, n) match {
      case (1, 2)           => List(L)
      case (2, 2)           => List(R)
  ...
  @tailrec
  private def subterm0(elems: List[PathElem], rest: Queue[(Int, Int)], mt: MTerm): MTerm =
    elems match {
...
case class ScLocal(i: Int) extends Name(s"{$i}")
...
case class EvalStmt[I](e: I) extends Stmt[I]
