...
trait PiPrinter extends Printer with PiAST {
  ...
  abstract override def print(p: Int, ii: Int, t: Term): Doc =
    t match {
      ...
      case Pi(d, r) =>
        parensIf(
          p > 0,
          sep(Seq("forall " <> parens(vars(ii) <> " : " <> print(0, ii, d)) <> " .", nest(print(0, ii + 1, r)))),
        )
      case Lam(t, Lam(t1, c)) =>
  ...
  private def nestedForall(i: Int, fs: List[(Int, Term)], t: Term): Doc =
    t match {
      case Pi(d, r) =>
  ...
  private def nestedLambda(i: Int, fs: List[(Int, Term)], t: Term): Doc =
    t match {
      case Lam(d, r) =>
  ...
  abstract override def printA(p: Int, ii: Int, t: Term): Doc =
    t match {
      ...
      case Pi(d, r) =>
        parensIf(
          p > 0,
          sep(Seq("forall " <> parens(vars(ii) <> " : " <> printA(0, ii, d)) <> " -> ", nest(printA(0, ii + 1, r)))),
        )
      case Lam(d, Lam(d1, r)) =>
      ...
      case Lam(d, r) =>
        parensIf(
          p > 0,
          sep(Seq("\\ " <> parens(vars(ii) <> " : " <> printA(0, ii, d)) <> " -> ", nest(printA(0, ii + 1, r)))),
        )
      case i :@: c =>
  ...
  abstract override def printC(p: Int, ii: Int, t: Term): Doc =
    t match {
      ...
      case Pi(d, r) =>
        parensIf(
          p > 0,
          sep(Seq("forall " <> parens(vars(ii) <> " : " <> printC(0, ii, d)) <> " ,", nest(printC(0, ii + 1, r)))),
        )
      case Lam(d, Lam(d1, r)) =>
        parensIf(p > 0, nestedLambda(ii + 2, List((ii + 1, d1), (ii, d)), r))
      case Lam(d, r) =>
        parensIf(
          p > 0,
          sep(Seq("fun " <> parens(vars(ii) <> " : " <> printC(0, ii, d)) <> " =>", nest(printC(0, ii + 1, r)))),
        )
      case i :@: c =>
