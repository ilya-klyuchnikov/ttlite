...
trait ListMetaSyntax extends MetaSyntax, ListAST {
  ...
  abstract override def translate(mt: MTerm): Term =
    mt match {
...
trait ListEval extends Eval, ListAST { self: PiAST =>
  ...
  def listElim(etVal: Value, mVal: Value, nilCaseVal: Value, consCaseVal: Value, listVal: Value): Value =
    listVal match {
      case VPiNil(_) =>
        nilCaseVal
...
trait ListQuoting extends CoreQuoting, ListAST {
  ...
  override def neutralQuote(ii: Int, n: Neutral): Term =
    n match {
